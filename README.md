...
- One-to-one and group chat (with group admin controls)
...
- **Chat System:**
   ...
   - Message status: sent, delivered, seen
   - Last seen/online status for users
...
- **Styling:** CSS modules in `src/styles/` (WhatsApp-inspired)
...
- WhatsApp-style group management UI (add/remove users, admin controls at top)
- Clean UI: All styles are in CSS files, avatars and layout
- Focused Navbar: Only "Users", "Profile", "Groups", and "Logout" (Logout on right)
- Real-time typing indicator: see "typing..." when the other user is typing in the same chat window
- Message status: sent (✔), delivered (✔✔), seen (blue ✔✔)
- Last seen/online status for users, auto-refreshes in chat window
